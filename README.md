...
**Honest curation note**: each topic has two tables. **✋ Hand-picked** = papers we chose ourselves — field classics plus papers we hit in our own experiments. **🤖 Agent-curated** = collected, classified and summarized by our AI-agent pipeline (arXiv + HF Daily, weekly) and **not yet read by us**. Deep vetting happens the honest way — when a paper actually enters our experiments. `Date` = publication (YYYY/MM); venue tag in `Tags` — no venue tag = arXiv preprint.
...
## 🛡️ Trustworthy
- [🛡️ Adversarial Robustness](collection/adversarial-robustness.md) (✋9+30)
...
2. Each topic opens with **✋ Hand-picked** papers we selected ourselves, followed by the **🤖 Agent-curated** pool.
3. We vet papers when we actually use them — no pretend review theater.

## 🗺️ The map

[![Paper map preview](docs/preview.png)](https://cubigcorp.github.io/awesome-data-centric-trustworthy-ai/)
...
> ✋ Each collection opens with a **Hand-picked** table — papers we selected ourselves (64 link-verified classics so far, plus whatever we hit in our own work) — followed by the 🤖 **Agent-curated** pool.

## Contributing
See [CONTRIBUTING.md](CONTRIBUTING.md). PRs welcome — format: `| Date | Title | Tags | TL;DR | 요약(KO) |`.

---
<sub>Maintained by [CUBIG](https://cubig.ai) · pilot 2026-07-23 · 241 papers (64 hand-picked + 177 agent-curated)</sub>
